...
const ManageAccount = ({ match }) => {
    ...
    useEffect(() => {
        // TBC: create endpoint to retrieve details about user
        axios.get(`${process.env.REACT_APP_BASEURL}/admins/${userID}?address=true`, {
            headers: {
    ...
    const editUserDetails = (onClose) => {
        setLoading(() => (true));
        axios.put(`${process.env.REACT_APP_BASEURL}/admins/${userID}`, {
