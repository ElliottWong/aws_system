    ...
    try {
        ...
        if (existingBlock) {
            ...
            if (existingBlock.status === DOCUMENT_STATUS.PENDING)
                throw new E.BlockingError('interested parties document', DOCUMENT_STATUS.PENDING);

            // rejected form below

            // there is a rejected form, and new form is not from the same employee
            if (existingBlock.created_by !== created_by)
                throw new E.BlockingError('interested parties document', DOCUMENT_STATUS.REJECTED);
        ...
        const { edit, employee: authorEmployee } = await findRights(created_by, companyId, 'm04_02');
        if (!edit) throw new E.PermissionError('edit');

        // check approver for appove rights
        const { approve, employee: approvingEmployee } = await findRights(approved_by, companyId, 'm04_02');
...
module.exports.rejectInterestedParties = async (req, res, next) => {
    try {
        ...
        const { approve } = await findRights(rejected_by, companyId, 'm04_02');
...
module.exports.approveInterestedParties = async (req, res, next) => {
    try {
        ...
        const { approve } = await findRights(approved_by, companyId, 'm04_02');
        if (!approve) throw new E.PermissionError('approve');
